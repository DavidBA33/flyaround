...
class ReviewController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $reviews = $em->getRepository('AppBundle:Review')->findAll(); // recuperation d'un tableau d'objets reviews
    ...
    {
        $review = new Review(); // je crée un nouvel objet $review
        $form = $this->createForm('AppBundle\Form\ReviewType', $review);

        $form->handleRequest($request); // reconnait que le formulaire a été soumis mais ne fait rien

        if ($form->isSubmitted() && $form->isValid()) {
            $em = $this->getDoctrine()->getManager();
            $em->persist($review); // prise en compte de l'objet $review par Doctrine (pas de requete SQL)
            $em->flush(); // Doctrine fait un INSERT dans la BDD
